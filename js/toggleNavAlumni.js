$('body').on('click', '.expandingNavClosed .headingexpandingAlumni', function(event){
  ...
  $(that).children('.expandingSubText').animate({
		...
		that.getElementsByClassName("headingexpandingAlumni")[0].getElementsByTagName("img")[0].src = "../images/slidingContent/minusSmall.png";
	  });
});

$('body').on('click', '.expandingNavOpen .headingexpandingAlumni', function(event){
  var that = this.parentNode;
  ...
  $(that).children('.expandingSubText').animate({
		...
		that.getElementsByClassName("headingexpandingAlumni")[0].getElementsByTagName("img")[0].src = "../images/slidingContent/plusSmall.png";
	  });
});
...
(function() {
	...
	for(node in nodes) {
		if(node < nodes.length) {
			var heading = nodes[node].getElementsByClassName("headingexpandingAlumni")[0];
			if(heading.getElementsByTagName("img").length == 0){
				innerData = nodes[node].getElementsByClassName("headingexpandingAlumni")[0].innerHTML
